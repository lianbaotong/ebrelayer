...
func createEvmTxSignViaHSM(secp256k1Index int, pub []byte, action proto.Message, execer, to string, fee int64) string {
	...
	r, s, err := adapter.SignSecp256k1(data, secp256k1Index)
	if nil != err {
...
func relayEvmTx2Chain33(signViaHsm bool, secp256k1Index int, pub []byte, privateKey chain33Crypto.PrivKey, claim *ebrelayerTypes.EthBridgeClaim, parameter, rpcURL, oracleAddr string) (string, error) {
	...
	var data string
	if signViaHsm {
		data = createEvmTxSignViaHSM(secp256k1Index, pub, &action, wholeEvm, toAddr, feeInt64)
	} else {
