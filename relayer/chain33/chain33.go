...
import (
	...
	syncTx "github.com/lianbaotong/ebrelayer/relayer/chain33/transceiver/sync"
...
func StartChain33Relayer(startPara *Chain33StartPara) *Relayer4Chain33 {
	chain33Relayer := &Relayer4Chain33{
		...
		matDegree:               startPara.SyncTxConfig.MaturityDegree,
	...
	if 0 == chain33Relayer.totalTx4RelayEth2chai33 {
		...
		_ = chain33Relayer.setLastestRelay2Chain33TxStatics(0, int32(events.ClaimTypeLock), data)
		_ = chain33Relayer.setLastestRelay2Chain33TxStatics(0, int32(events.ClaimTypeBurn), data)
	}
...
func (chain33Relayer *Relayer4Chain33) relayLockBurnToChain33(claim *ebTypes.EthBridgeClaim) {
	...
	if chain33Relayer.signViaHsm {
		...
		R, S, err := adapter.SignSecp256k1Workaround(utils.SoliditySHA3WithPrefix(claimID[:]), chain33Relayer.secp256k1Index, address.PubKeyToAddress(chain33Relayer.chain33PubKey).String(), adapter.AddrVerifyTypeChain33)
		if nil != err {
