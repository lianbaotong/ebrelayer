...
func StartEthereumRelayer(startPara *EthereumStartPara) *Relayer4Ethereum {
	...
	ethRelayer := &Relayer4Ethereum{
		...
		ethSender:               common.HexToAddress(startPara.Validator),
...
func (ethRelayer *Relayer4Ethereum) handleChain33Msg(chain33Msg *events.Chain33Msg) {
	...
	txhash, err := ethtxs.RelayOracleClaimToEthereum(ethRelayer.x2EthContracts.Oracle, ethRelayer.clientSpec, ethRelayer.ethSender, tokenAddr, prophecyClaim,
		ethRelayer.privateKey4Ethereum, ethRelayer.signViaHsm, ethRelayer.secp256k1Index)
	if nil != err {
