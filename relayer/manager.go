...
import (
	...
	"github.com/lianbaotong/ebrelayer/relayer/chain33"
	"github.com/lianbaotong/ebrelayer/relayer/ethereum"
	relayerTypes "github.com/lianbaotong/ebrelayer/types"
...
func (manager *Manager) Unlock(passphase string, result *interface{}) error {
	...
	if err := manager.ethRelayer.RestorePrivateKeyOrPasspin(passphase); nil != err {
...
//ImportChain33RelayerPrivateKey 导入chain33relayer验证人的私钥,该私钥实际用于向chain33提交验证交易时签名使用
func (manager *Manager) ImportChain33RelayerPrivateKey(importKeyReq relayerTypes.ImportKeyReq, result *interface{}) error {
	...
	if manager.chain33Relayer.IsSignViaHsm() {
		mlog.Error("ImportPrivateKeyPasspin4chain33", "Failed due to: ", "chain33 relayer is configured to sign via HSM")
...
func (manager *Manager) ImportEthereumPrivateKeyPasspin(privateKeyPasspin string, result *interface{}) error {
	...
	err := manager.ethRelayer.ImportPrivateKeyPasspin(manager.passphase, privateKeyPasspin)
	if err != nil {
		mlog.Error("ImportEthereumPrivateKey4EthRelayer", "Failed due to cause:", err.Error())
		return err
	}
